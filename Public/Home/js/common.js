$(function(){
	...
	function login_dialog(){
	    CUR_DIALOG = ajax_form('login','登录',SITEURL+'/index.php?act=denglu&inajax=1',360,1);
	}
...
//	    $.get('index.php?act=shouye&op=denglu', function(result){
//	        if(result=='0'){
//	            login_dialog();
//	        }else{
//	            var url = 'index.php?act=member_huiyuan_aihao&op=favoritesstore';
...
//		
//	    $.get('index.php?act=index&op=denglu', function(result){
//	        if(result=='0'){
//	            login_dialog();
//	        }else{
//	            var url = 'index.php?act=member_huiyuan_aihao&op=favoritesgoods';
//	          
//	            $.getJSON(url, {'fid':fav_id}, function(data){
//	                if (data.done)
//	            {
...
//	}
	...
	function load_cart_information(){
		$.getJSON(SITEURL+'/index.php?act=member_gouwuche&op=ajax_load&callback=?', function(result){
	...
	function drop_topcart_item(cart_id,goods_id){
	    $.getJSON(SITEURL+'/index.php?act=member_gouwuche&op=del&cart_id='+cart_id+'&goods_id='+goods_id+'&callback=?', function(result){
	        if(result.state){
	            var obj = $('.head-user-menu .my-cart');
	            //删除成功
	...
	(function($) {
	    $.show_nc_login = function(options) {
	        var settings = $.extend({}, {action:'/index.php?act=denglu&op=login&inajax=1', nchash:'', formhash:'' ,anchor:''}, options);
	        ...
	        login_dialog_html.append('<form class="bg" method="post" id="ajax_login" action="'+APP_SITE_URL+settings.action+'"></form>').find('form')
	        	...
	        	.append('<dl><dt>验证码</dt><dd><input type="text" size="10" maxlength="4" class="text fl w60" name="captcha"><img border="0" onclick="this.src=\''+SHOP_SITE_URL+'/index.php?act=yanzhengma&amp;op=makecode&amp;nchash='+settings.nchash+'&amp;t=\' + Math.random()" name="codeimage" title="看不清，换一张" src="'+SHOP_SITE_URL+'/index.php?act=yanzhengma&amp;op=makecode&amp;nchash='+settings.nchash+'" class="fl ml10"><span>不区分大小写</span></dd></dl>')
	        	.append('<ul><li>›&nbsp;如果您没有登录帐号，请先<a class="register" href="'+SHOP_SITE_URL+'/index.php?act=denglu&amp;op=register">注册会员</a>然后登录</li><li>›&nbsp;如果您<a class="forget" href="'+SHOP_SITE_URL+'/index.php?act=login&amp;op=forget_password">忘记密码</a>？，申请找回密码</li></ul>')
	...
	(function($){
		$.fn.membershipCard = function(options){
			...
			return this.each(function(){
				...
				var _uri = SITEURL+'/index.php?act=member_kapian&callback=?&uid='+data_str.id+'&from='+options.type;
				$this.qtip({
		            content: {
		                text: 'Loading...',
		                ajax: {
			                ...
			                success: function(data) {
			                	if(data){
			                		...
			                		$('<dt class="member-id"></dt>').append('<i class="sex'+data.sex+'"></i>')
			                			.append('<a href="'+SHOP_SITE_URL+'/index.php?act=member_huiyuan_shequzhuye&mid='+data.id+'" target="_blank">'+data.name+'</a>'+(data.truename != ''?'('+data.truename+')':''))
			                			.appendTo(_dl);
			                		// avatar
			                		$('<dd class="avatar"><a href="'+SHOP_SITE_URL+'/index.php?act=member_huiyuan_shequzhuye&mid='+data.id+'" target="_blank"><img src="'+data.avatar+'" /></a><dd>')
			                			.appendTo(_dl);
			                		// info
			                		var _info = '';
			                		...
			                		if(data.follow != 2){
			                			...
			                			$('<div class="send-msg"> <a href="'+SHOP_SITE_URL+'/index.php?act=member_zhuye&op=sendmsg&member_id='+data.id+'" target="_blank"><i></i>站内信</a> </div>').appendTo(_bottom);
			                		}
			...
			function follow(o){
				...
				$.getJSON(SHOP_SITE_URL+'/index.php?act=member_huiyuan_shequpengyou&op=addfollow&callback=?&mid='+data_str.mid, function(data){
					if(data){
			...
			function onfollow(o){
				var data_str = o.parent().attr('data-param');
				eval( "data_str = "+data_str);
				$.getJSON(SHOP_SITE_URL+'/index.php?act=member_huiyuan_shequpengyou&op=delfollow&callback=?&mid='+data_str.mid, function(data){
					if(data){
